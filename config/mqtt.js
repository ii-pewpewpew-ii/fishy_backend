...
const handleMessage = async(parsedMessage)=>{
    ...
    if(contents[0] === 'G'){
        ...
        fishingTrip.tripStartHandler(parsedMessage.from)
    }else if(contents[0] === 'Trip' && contents[1] === 'ended'){
        fishingTrip.tripEndHandler(parsedMessage.from);
    }else if(contents[0] === 'S'){
        fishingTrip.handleFishingZoneSuggestionPing({
